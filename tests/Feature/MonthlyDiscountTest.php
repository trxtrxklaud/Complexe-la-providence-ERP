...
class MonthlyDiscountTest extends TestCase
{
    ...
    /**
     * Test 47: Generalized double counting test for multiple months, multiple levels, and club fees.
     * Proves that any combination of months (1, 2, 3) and fixed discounts does not double count the net total.
     */
    public function test_47_generalized_double_counting_prevention(): void
    {
        $user = $this->makeWaiverUser();
        Sanctum::actingAs($user);
        ...
        // 1. Level with 180 TND/month fee, 2 months -> 360 TND. No discount.
        $enrollmentL5 = $this->makeEnrollment($year);
        $this->makeFeePlan($year, $enrollmentL5->level_id, 180.0);

        $tuitionFee = \App\Models\FeeType::create(['name_ar' => 'القسط', 'price' => 180.0, 'is_active' => true]);

        $response1 = $this->postJson('/api/payments/collect', [
            'student_id' => $enrollmentL5->student_id,
            'enrollment_id' => $enrollmentL5->id,
            'months' => ['2025-09', '2025-10'],
            'payment_date' => now()->toDateString(),
            'method' => 'cash',
            'items' => [
                ['fee_type_id' => $tuitionFee->id, 'amount' => 360.0],
            ],
        ]);

        $response1->assertCreated()->assertJsonPath('receipt.total', 360);

        // 2. Level with 160 TND/month fee, 3 months -> 480 TND. Fixed discount 15 TND/month (total 45). Net = 435.
        $enrollmentL3 = $this->makeEnrollment($year);
        $this->makeFeePlan($year, $enrollmentL3->level_id, 160.0);
        app(DiscountService::class)->createForEnrollment($enrollmentL3->id, 15.0, 'خصم ثابت', '2025-09-01');

        $clubFee = \App\Models\FeeType::create(['name_ar' => 'نادي', 'price' => 50.0, 'is_active' => true]);

        // Collect 3 months (435 TND net tuition) + 1 club fee (50 TND) = 485 TND.
        $response2 = $this->postJson('/api/payments/collect', [
            'student_id' => $enrollmentL3->student_id,
            'enrollment_id' => $enrollmentL3->id,
            'months' => ['2025-09', '2025-10', '2025-11'],
            'payment_date' => now()->toDateString(),
            'method' => 'bank_transfer',
            'items' => [
                ['fee_type_id' => $tuitionFee->id, 'amount' => 435.0],
                ['fee_type_id' => $clubFee->id, 'amount' => 50.0],
            ],
        ]);

        $response2->assertCreated()->assertJsonPath('receipt.total', 485);

        // 3. One month selection shouldn't change calculation basis
        $response3 = $this->postJson('/api/payments/collect', [
            'student_id' => $enrollmentL3->student_id,
            'enrollment_id' => $enrollmentL3->id,
            'months' => ['2025-12'],
            'payment_date' => now()->toDateString(),
            'method' => 'check',
            'items' => [
                ['fee_type_id' => $tuitionFee->id, 'amount' => 145.0],
            ],
        ]);

        $response3->assertCreated()->assertJsonPath('receipt.total', 145);
    }
}
