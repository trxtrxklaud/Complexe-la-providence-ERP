...
export function PageDataSkeleton({ cards = 4, rows = 5 }: PageDataSkeletonProps) {
  return (
    <div className="space-y-4" role="status" aria-label="تحميل البيانات">
      <div className="grid gap-3" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(11rem, 1fr))' }}>
        {Array.from({ length: cards }).map((_, index) => (
          <div key={index} className="skeleton-shimmer h-24 rounded-2xl border border-[#EDF1E8] bg-white/80 p-4">
            <div className="h-3 w-24 rounded bg-slate-200" />
            <div className="mt-4 h-6 w-32 rounded bg-slate-300/70" />
          </div>
        ))}
      </div>
      <div className="skeleton-shimmer overflow-hidden rounded-2xl border border-[#EDF1E8] bg-white/80">
        ...
        <tr key={rowIndex} aria-hidden="true">
          {Array.from({ length: columns }).map((_, columnIndex) => (
            <td key={columnIndex} className="px-6 py-4">
              <div className={`skeleton-shimmer h-3 rounded bg-slate-200 ${columnIndex % 3 === 0 ? 'w-16' : 'w-28'}`} />
            </td>
          ))}
        </tr>
      ))}
...
export function ListSkeleton({ rows = 6 }: { rows?: number }) {
  return (
    <div className="divide-y divide-[#EDF1E8]" role="status" aria-label="تحميل القائمة">
      {Array.from({ length: rows }).map((_, index) => (
        <div key={index} className="skeleton-shimmer flex items-center gap-4 p-5">
